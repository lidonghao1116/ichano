 ...
 *
 */
public class BlowfishEncodingFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if (isEncrypt) {
			...
			BlowfishRequestWrapper requestWrapper = null;
			if (httpRequest instanceof HttpServletRequest) {
					requestWrapper = new BlowfishRequestWrapper(
							(HttpServletRequest) request);
			}

			BlowfishResponseWrapper wrapper = new BlowfishResponseWrapper(
					httpResponse);
			chain.doFilter(requestWrapper, wrapper);
			if (wrapper.getStatus() == HttpServletResponse.SC_OK&&requestWrapper.getMethod().equals("POST")) {
				// 对响应进行处理，
